...
from mono.datasets.loader import build_dataloader
...
def batch_processor(model, data, train_mode):
    ...
    outputs = dict(loss=loss,
                   log_vars=log_vars,
                   num_samples=data['leftImage'].size()[0])
...
def _dist_train(model, dataset_train, dataset_val, cfg, validate=False):
    ...
    if validate:
        ...
        runner.register_hook(DistEvalMonoHook(dataset_val, interval))
...
def _non_dist_train(model, dataset_train, dataset_val, cfg, validate=True):
    # prepare data loaders
    data_loaders = [
        build_dataloader(dataset_train,
